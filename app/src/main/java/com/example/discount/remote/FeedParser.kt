...
class FeedParser {
    ...
    private fun readFeedItem(parser: XmlPullParser): FeedItem {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            } else if (parser.eventType == XmlPullParser.START_TAG && parser.name == "author") {
                author = readText(parser)
            } else if (parser.eventType == XmlPullParser.START_TAG && parser.name == "pubDate") {
                pubDate = readText(parser)
            } else if (parser.eventType == XmlPullParser.START_TAG && parser.name == "image") {
                image = readText(parser)
            }else if (parser.eventType == XmlPullParser.START_TAG && parser.name == "media:group"){
                image = readText(parser)
            }
            else if (parser.eventType == XmlPullParser.START_TAG && parser.name == "media:content"){
